// Vergic/vngage change: Note that this is a Vergic modified version of SignalR client lib v3.0.0
// This version does not polyfill the global window.Promise (as that may break customer's websites if they are monkey-patching it with alternative Promise-implementations, etc)
// And we also do not want to risk that our SignalR-lib is using any weird implementations...
// Search for "Vergic/vngage" to find modifications
...
// Vergic/vngage change: Create a locally scoped "Promise"-var that SignalR in this module will use instead of the globally scoped "window.Promise".
// Init to window.Promise, but it may be overridden (instead of polyfilling "window.Promise"!) to "es6-promise" later if needed
// if "window.Promise" is missing (IE11) or if shimmed/monkey-patched on the customer's website (like CityFurniture do)
...
function polyfill() {
  ...
  // Vergic/vngage change: Check this module's locally scoped "Promise" instead of "local.Promise" (i.e. window.Promise in a browser)!
  // var P = local.Promise;
  var P = Promise;
  ...
  // Vergic/vngage change: Don't polyfill "local.Promise" (i.e. window.Promise in a browser)!
  // That may break the customer's site (if they are mokey-patching window.Promise to any weird custom implementation)
...
// Vergic/vngage change: Removed sourceMapping link
...
var VERSION = "5.0.5 (Vergic mod)"; // Vergic/vngage change: Note that this is a Vergic modified version
/** @private */
